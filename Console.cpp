...
void SetConsole(Console& c) {
	LPCTSTR s = _T("CASSE BRIQUES");
	c.set_title(s);
	c.set_dimensions(59, 35);
	c.set_cursor_position(0, 0);
	c.set_pen_color(yellow);
}

bool bienvenu(Console& c) {
	c.put_string("CASSE BRIQUES", 23, 2);
	char str[30];
	for (int i = 0; i < 30; i++) {
		str[i] = 0xFEu;
		if (i == 29) str[i] = '\0';
	...
	c.put_string(str, 0, 5);
	c.put_string(str, 0, 19);
	string s = "En bas de l'ecran se trouve une barre que vous deplacez    vers la droite (->) ou vers la gauche (<-) avec les touches correspondantes du clavier. La balle rebondit sur la barre quand elle « tombe » dessus.Il y a trois types de briques differentes.Lesquelles se trouvent en haut, a gauche et a  droit seront cassees quand la balle en tombe dessus 2 fois. (Ils devraient verts le premier fois.)Par contre, si la   balle atteint le bas de l'ecran le jeu est perdu. Si toutes les briques sont cassees, vous gagnez.";
	c.set_pen_color(yellow);
	c.put_string("   INTRODUCTION", 20, 6);
	c.put_string("Vous voulez commencer le jeu?", 15, 21);
	c.put_string(s, 0, 8);
	c.put_string("oui<-", 20, 22);
	c.put_string("->Non", 29, 22);
	c.set_cursor_position(26, 22);
welcome:
	while (!_kbhit()) { if (_kbhit())  break; }
	int ch = _getch();
	switch (ch) {
	case(75): return 1;
	case(77): {c.clear(); c.set_cursor_position(20, 20); std::cout << "GAME OVER! "; return 0; }
	default: goto welcome;
	}
}

bool isterminer(Console& c) {
	c.set_cursor_position(20, 20);
	cout << "Vous voulez recommencer?";
	c.put_string("oui<-", 20, 22);
	c.put_string("->Non", 29, 22);
	c.set_cursor_position(24, 22);
end:
	while (!_kbhit()) { if (_kbhit())  break; }
	int ch = _getch();
	switch (ch) {
	case(75): c.clear(); return 0;
	case(77): { return 1; }
	default: goto end;
	}
}
