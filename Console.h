...
/*class barreConsole : public Console {
public:
	Barre* barre;
	barreConsole(Barre b):barre(&b) {}
	void onKeyPressed(WORD keyCode) {
		switch (keyCode)
		{
		case('a'): {if (barre->position.x > 2) barre->position.x--; break;}
		case('d'): {if (barre->position.x < 55) barre->position.x++; break;}
		default:break;
		}
	}
};*/
